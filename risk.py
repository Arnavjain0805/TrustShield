...
def predict_login(username, ip, device, browser, hour, mfa_enabled,
    ...
    # 0. Defense Layer Pre-Check
    try:
        severity, is_suspicious, def_reasons = evaluate_input_robustness(ip, device, browser, hour)
        if severity == "BLOCK":
            return 100, "BLOCK ❌", def_reasons
        elif severity == "MFA":
            return 50, "ALLOW with MFA 🔐", def_reasons
    except Exception:
        pass
